...
public class GameImage extends Component {
	...
	public GameImage(String s) {
		try {
			/*
			 * image1 = ImageIO.read(getClass().getResourceAsStream(path + s +
			 * ".png")); //Low res used for outline image2 =
			 * ImageIO.read(getClass().getResourceAsStream(path + s + "2.png"));
			 * //High res used for drawing
			 */
	...
	public GameImage(String s, int w, int h) {
		try {
			/*
			 * image1 = ImageIO.read(SpaceImage.class.getResourceAsStream(path +
			 * s + ".png")); //Low res used for outline image2 =
			 * ImageIO.read(SpaceImage.class.getResourceAsStream(path + s +
			 * "2.png")); //High res used for drawing
			 */
			image1 = ImageIO.read(getClass().getResource(path + s + ".png"));
			image2 = ImageIO.read(getClass().getResource(path + s + "2.png"));
		} catch (Exception e) {
		...
		try {
			...
			for (int y = 0; y < image1.getHeight() / h; y++) {
				for (int x = 0; x < image1.getWidth() / w; x++) {
					generate(outlines.get(y * (image1.getWidth() / w) + x),
							width, height);
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Overloaded method, allowing a sprite sheet to return multiple outlines.
	 * Supports up to 10 outlines.
	 * 
	 * @param o
	 *            Int outline to return.
	...
	public BufferedImage getImage(int xpos, int ypos) {
		...
		BufferedImage image = image2.getSubimage(xpos * width, ypos * height,
				width, height);
		return image;
	}
